...
impl StateHE {
    ...
    pub fn RatchetEncryptHE(&mut self, plaintext: Vec<u8>, AD: Vec<u8>) -> Vec<u8> {
        ...
        let enc_header = Self::HENCRYPT(self.HCT, self.HKs.clone(), header);
        self.HCT += 1;
    ...
    fn HENCRYPT(hct: u128, hk: Vec<u8>, plaintext: Vec<u8>) -> Vec<u8> {
        use aes::Aes256;
        ...
        if hct >= u128::MAX - 1 {
            panic!("HEADER NONCE OVERFLOW!!1!111!");
        }
        let hct = hct.to_le_bytes().to_vec();
        for i in 0..16 {
            noncebytes[i] = hct[i];
        }
